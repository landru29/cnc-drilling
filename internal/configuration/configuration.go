...
type Config struct {
	...
	DeepStart    float64         `default:"0"      json:"deep_start"     mapstructure:"deep_start"    yaml:"deep_start"`
...
func (c Config) TryDeeps() []float64 {
	...
	output := make([]float64, int(math.Ceil((c.Deepness-c.DeepStart)/c.DeepPerTry)))

	maxFullTry := int(math.Floor((c.Deepness - c.DeepStart) / c.DeepPerTry))

	output[len(output)-1] = math.Mod(c.Deepness-c.DeepStart, c.DeepPerTry) + float64(maxFullTry)*c.DeepPerTry + c.DeepStart

	for index := range maxFullTry {
		output[index] = float64(index+1)*c.DeepPerTry + c.DeepStart
	}
